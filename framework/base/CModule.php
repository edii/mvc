...
abstract class CModule extends \CComponent
{
	...
	public function __construct($id, $parent, $config=null)
	{
		...
		\init::setPathOfAlias($id,$this->getBasePath());
	...
	public function setBasePath($path) {
		if(($this->_basePath = realpath($path)) === false || !is_dir( $this->_basePath ))
			throw new \CException(\init::t('init','Base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setModulePath($value) {
		if(($this->_modulePath = realpath($value)) === false || !is_dir($this->_modulePath))
			throw new \CException(\init::t('init','The module path "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setAliases($mappings) {
		foreach($mappings as $name=>$alias) {
			if(($path = \init::getPathOfAlias($alias))!==false)
				\init::setPathOfAlias($name,$path);
			else
				\init::setPathOfAlias($name,$alias);
		}
	}
	...
	public function getModule($id) {
		...
		elseif(isset($this->_moduleConfig[$id])) {
			...
			if(!isset($config['enabled']) || $config['enabled']) {
				\init::trace("Loading \"$id\" module",'system.base.CModule');
				$class=$config['class'];
				unset($config['class'], $config['enabled']);
				if($this===\init::app())
					$module=\init::createComponent($class,$id,null,$config);
				else
					$module=\init::createComponent($class,$this->getId().'/'.$id,$this,$config);
	...
	public function setModules($modules) {
		foreach($modules as $id=>$module) {
			...
			if(!isset($module['class'])) {
				\init::setPathOfAlias($id,$this->getModulePath().DIRECTORY_SEPARATOR.$id);
	...
	public function getComponent($id,$createIfNull=true) {
		...
		elseif(isset($this->_componentConfig[$id]) && $createIfNull) {
			...
			if(!isset($config['enabled']) || $config['enabled']) {
				\init::trace("Loading \"$id\" application component",'system.CModule');
				unset($config['enabled']);
				$component=\init::createComponent($config);
				$component->init();
	 ...
	 *         'class'=>'CDbCache',
	 *         'connectionID'=>'db',
	 *         'enabled'=>!DEBUG,  // enable caching in non-debug mode
	 *     ),
	 * )
	 * </pre>
	 *
