...
class CClientScript extends \CApplicationComponent
{
	...
	public function getCoreScriptUrl()
	{
		if($this->_baseUrl!==null)
			...
			return $this->_baseUrl=\init::app()->getAssetManager()->publish(PATH.'/web/js/source');
	}
	...
	public function getPackageBaseUrl($name)
	{
		...
		if(isset($package['baseUrl']))
		{
			...
			if($baseUrl==='' || $baseUrl[0]!=='/' && strpos($baseUrl,'://')===false)
				$baseUrl=\init::app()->getRequest()->getBaseUrl().'/'.$baseUrl;
			$baseUrl=rtrim($baseUrl,'/');
		}
		elseif(isset($package['basePath']))
			$baseUrl=\init::app()->getAssetManager()->publish(\init::getPathOfAlias($package['basePath']));
		else
	...
	public function registerCoreScript($name)
	{
		...
		else
		{
			if($this->corePackages===null)
				$this->corePackages=require(PATH.'/web/js/packages.php');
	 ...
	 * This is a shortcut to \init::app()->controller->recordCachingAction.
	...
	{
		if(($controller=\init::app()->getController())!==null)
