...
class CHttpRequest extends \CApplicationComponent
{
	 ...
	 * By setting this property to true, forms submitted to an Init Web application must be originated
	...
	/**
	 * @var string the name of the token used to prevent CSRF. Defaults to 'CSRF_TOKEN'.
	...
	protected function normalizeRequest()
	{
		...
		if($this->enableCsrfValidation)
			\init::app()->attachEventHandler('onBeginRequest',array($this,'validateCsrfToken'));
	}
	...
	public function getScriptUrl()
	{
		if($this->_scriptUrl===null)
		{
			...
			elseif(isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'],$_SERVER['DOCUMENT_ROOT'])===0)
				...
				throw new \CException(\init::t('init','CHttpRequest is unable to determine the entry script URL.'));
		}
	...
	public function getPathInfo()
	{
		if($this->_pathInfo===null)
		{
			...
			elseif(strpos($_SERVER['PHP_SELF'],$scriptUrl)===0)
				...
				throw new \CException(\init::t('init','CHttpRequest is unable to determine the path info of the request.'));
	...
	public function validateCsrfToken($event)
	{
			...
			$this->getIsDeleteRequest())
		{
			...
			if (!empty($userToken) && $cookies->contains($this->csrfTokenName))
			{
				...
				$valid = false;
			if (!$valid)
				throw new \CHttpException(400,\init::t('init','The CSRF token could not be verified.'));
		}
	}
}
...
class CCookieCollection extends \CMap
{
	...
	protected function getCookies()
	{
		...
		{
			$sm=\init::app()->getSecurityManager();
	...
	public function add($name,$cookie)
	{
		if($cookie instanceof \CHttpCookie)
		{
			...
			throw new \CException(\init::t('init','CHttpCookieCollection can only hold CHttpCookie objects.'));
	}
	...
	protected function addCookie($cookie)
	{
		$value=$cookie->value;
		if($this->_request->enableCookieValidation)
			$value=\init::app()->getSecurityManager()->hashData(serialize($value));
