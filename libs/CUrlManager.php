...
{
	const CACHE_KEY='init.CUrlManager.rules';
	...
	protected function processRules()
	{
		if(empty($this->rules) || $this->getUrlFormat()===self::GET_FORMAT)
			return;
		if($this->cacheID!==false && ($cache=\init::app()->getComponent($this->cacheID))!==null)
		{
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		{
			if(is_array($rule))
				$this->_rules[$i]=$rule=\init::createComponent($rule);
	...
	public function getBaseUrl()
	{
		...
		{
			if($this->showScriptName)
				$this->_baseUrl=\init::app()->getRequest()->getScriptUrl();
			else
				$this->_baseUrl=\init::app()->getRequest()->getBaseUrl();
			return $this->_baseUrl;
		}
	}
	...
	public function setUrlFormat($value)
	{
		if($value===self::PATH_FORMAT || $value===self::GET_FORMAT)
			...
			throw new \CException(\init::t('init','CUrlManager.UrlFormat must be either "path" or "get".'));
	}
}
...
class CUrlRule extends \CBaseUrlRule
{
	...
	public function __construct($route,$pattern)
	{
		...
		if(DEBUG && @preg_match($this->pattern,'test')===false)
			throw new \CException(\init::t('init','The URL pattern "{pattern}" for route "{route}" is not a valid regular expression.',
	...
	public function createUrl($manager,$route,$params,$ampersand)
	{
		...
		if($this->hasHostInfo)
		{
			$hostInfo=\init::app()->getRequest()->getHostInfo();
