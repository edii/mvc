...
class CI_DB_driver {
	...
	function display_error($error = '', $swap = '', $native = FALSE)
	{
		...
		{
			if (isset($call['file']) and strpos($call['file'], PATH_LIBS.'database') === FALSE)
			{
				// Found it - use a relative path for safety
				$message[] = 'Filename: '.str_replace(array(PATH_LIBS, APPPATH), '', $call['file']);
	...
	function _protect_identifiers($item, $prefix_single = FALSE, $protect_identifiers = NULL, $field_exists = TRUE)
	{
		...
		if (strpos($item, '.') !== FALSE)
		{
			...
			if ($this->dbprefix != '')
			{
				...
				// Verify table prefix and replace if necessary
				if ($this->swap_pre != '' and strncmp($parts[$i], $this->swap_pre, strlen($this->swap_pre)) === 0)
				{
		...
		{
			// Verify table prefix and replace if necessary
			if ($this->swap_pre != '' and strncmp($item, $this->swap_pre, strlen($this->swap_pre)) === 0)
			{
